...
#include "./new/vulkanApp.h"

using namespace vkx;

vulkanApp::vulkanApp(bool enableValidation) : swapChain(*this) {
...
int32_t vulkanApp::handleAppInput(struct android_app* app, AInputEvent* event) {
    vulkanApp* vulkanExample = (vulkanApp*)app->userData;
...
void vulkanApp::KeyboardHandler(GLFWwindow* window, int key, int scancode, int action, int mods) {
    vulkanApp* example = (vulkanApp*)glfwGetWindowUserPointer(window);
...
void vulkanApp::MouseHandler(GLFWwindow* window, int button, int action, int mods) {
    vulkanApp* example = (vulkanApp*)glfwGetWindowUserPointer(window);
    if (action == GLFW_PRESS) {
...
void vulkanApp::MouseMoveHandler(GLFWwindow* window, double posx, double posy) {
    vulkanApp* example = (vulkanApp*)glfwGetWindowUserPointer(window);
    example->mouseMoved(glm::vec2(posx, posy));
}

void vulkanApp::MouseScrollHandler(GLFWwindow* window, double xoffset, double yoffset) {
    vulkanApp* example = (vulkanApp*)glfwGetWindowUserPointer(window);
    example->mouseScrolled(yoffset);
}


void vulkanApp::CloseHandler(GLFWwindow* window) {
    vulkanApp* example = (vulkanApp*)glfwGetWindowUserPointer(window);
    example->prepared = false;
    glfwSetWindowShouldClose(window, 1);
}

void vulkanApp::FramebufferSizeHandler(GLFWwindow* window, int width, int height) {
    vulkanApp* example = (vulkanApp*)glfwGetWindowUserPointer(window);
