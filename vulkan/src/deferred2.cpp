...
//		camera.setPerspective(60.0f, (float)width / (float)height, 0.1f, 256.0f);
	...
	~VulkanExample()
	{
		...
		//// Meshes
		////vkx::freeMeshBufferResources(device, &meshes.model);
		////vkx::freeMeshBufferResources(device, &meshes.floor);
		////vkx::freeMeshBufferResources(device, &meshes.quad);
	...
	void createAttachment(vk::Format format, vk::ImageUsageFlagBits usage, FrameBufferAttachment *attachment)
	{
		...
		vk::ImageCreateInfo image;// = vkx::initializers::imageCreateInfo();
		...
		vk::MemoryAllocateInfo memAlloc;// = vkx::initializers::memoryAllocateInfo();
		vk::MemoryRequirements memReqs;

		attachment->image = device.createImage(image, nullptr);
		memReqs = device.getImageMemoryRequirements(attachment->image);

		memAlloc.allocationSize = memReqs.size;
		memAlloc.memoryTypeIndex = vulkanDevice->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		
		attachment->mem = device.allocateMemory(memAlloc, nullptr);
		device.bindImageMemory(attachment->image, attachment->mem, 0);


		vk::ImageViewCreateInfo imageView;// = vkx::initializers::imageViewCreateInfo();
		...
		attachment->view = device.createImageView(imageView, nullptr);
	}
	...
	void prepareOffscreenFramebuffer()
	{
		...
		vk::Bool32 validDepthFormat = vkx::getSupportedDepthFormat(physicalDevice, &attDepthFormat);
		...
		offScreenFrameBuf.renderPass = device.createRenderPass(renderPassInfo, nullptr);
		...
		vk::SamplerCreateInfo sampler;// = vkx::initializers::samplerCreateInfo();
		...
		colorSampler = device.createSampler(sampler, nullptr);
	}
	...
	void buildDeferredCommandBuffer()
	{
		...
		vk::SemaphoreCreateInfo semaphoreCreateInfo;// = vkx::initializers::semaphoreCreateInfo();
		offscreenSemaphore = device.createSemaphore(semaphoreCreateInfo, nullptr);

		vk::CommandBufferBeginInfo cmdBufInfo;// = vkx::initializers::commandBufferBeginInfo();
		...
		vk::RenderPassBeginInfo renderPassBeginInfo = vkx::initializers::renderPassBeginInfo();
		...
		vk::Viewport viewport = vkx::initializers::viewport((float)offScreenFrameBuf.width, (float)offScreenFrameBuf.height, 0.0f, 1.0f);
		vkCmdSetViewport(offScreenCmdBuffer, 0, 1, &viewport);

		vk::Rect2D scissor = vkx::initializers::rect2D(offScreenFrameBuf.width, offScreenFrameBuf.height, 0, 0);
		...
		// Background
		vkCmdBindDescriptorSets(offScreenCmdBuffer, vk::PipelineBindPoint::eGraphics, pipelineLayouts.offscreen, 0, 1, &descriptorSets.floor, 0, NULL);
		...
		// Object
		vkCmdBindDescriptorSets(offScreenCmdBuffer, vk::PipelineBindPoint::eGraphics, pipelineLayouts.offscreen, 0, 1, &descriptorSets.model, 0, NULL);
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			vk::Viewport viewport = vkx::initializers::viewport((float)width, (float)height, 0.0f, 1.0f);
			vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);

			vk::Rect2D scissor = vkx::initializers::rect2D(width, height, 0, 0);
	...
	void setupVertexDescriptions()
	{
		...
		vertices.bindingDescriptions[0] =
			vkx::initializers::vertexInputBindingDescription(
		...
		vertices.attributeDescriptions[0] =
			vkx::initializers::vertexInputAttributeDescription(
				VERTEX_BUFFER_BIND_ID,
				0,
		...
		vertices.inputState = vkx::initializers::pipelineVertexInputStateCreateInfo();
	...
	void setupDescriptorPool()
	{
		...
		{
			vkx::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 8),
			vkx::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 9)
		};

		vk::DescriptorPoolCreateInfo descriptorPoolInfo =
			vkx::initializers::descriptorPoolCreateInfo(
	...
	void setupDescriptorSetLayout()
	{
		...
		std::vector<vk::DescriptorSetLayoutBinding> setLayoutBindings =
		{
			...
			vkx::initializers::descriptorSetLayoutBinding(
		...
		vk::DescriptorSetLayoutCreateInfo descriptorLayout =
			vkx::initializers::descriptorSetLayoutCreateInfo(
		...
		vk::PipelineLayoutCreateInfo pPipelineLayoutCreateInfo =
			vkx::initializers::pipelineLayoutCreateInfo(
				&descriptorSetLayout,
				1);
	...
	void setupDescriptorSet()
	{
		...
		vk::DescriptorSetAllocateInfo allocInfo =
			vkx::initializers::descriptorSetAllocateInfo(
				descriptorPool,
				&descriptorSetLayout,
				1);
		...
		vk::DescriptorImageInfo texDescriptorPosition =
			vkx::initializers::descriptorImageInfo(
				colorSampler,
		...
		writeDescriptorSets = {
			// Binding 0 : Vertex shader uniform buffer
			vkx::initializers::writeDescriptorSet(
				descriptorSet,
				VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
				0,
	...
	void preparePipelines()
	{
		vk::PipelineInputAssemblyStateCreateInfo inputAssemblyState =
			vkx::initializers::pipelineInputAssemblyStateCreateInfo(
				VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST,
				0,
				VK_FALSE);

		vk::PipelineRasterizationStateCreateInfo rasterizationState =
			vkx::initializers::pipelineRasterizationStateCreateInfo(
		...
		vk::PipelineColorBlendAttachmentState blendAttachmentState =
			vkx::initializers::pipelineColorBlendAttachmentState(
				0xf,
				VK_FALSE);

		vk::PipelineColorBlendStateCreateInfo colorBlendState =
			vkx::initializers::pipelineColorBlendStateCreateInfo(
				1,
				&blendAttachmentState);

		vk::PipelineDepthStencilStateCreateInfo depthStencilState =
			vkx::initializers::pipelineDepthStencilStateCreateInfo(
				VK_TRUE,
				VK_TRUE,
				VK_COMPARE_OP_LESS_OR_EQUAL);

		vk::PipelineViewportStateCreateInfo viewportState =
			vkx::initializers::pipelineViewportStateCreateInfo(1, 1, 0);

		vk::PipelineMultisampleStateCreateInfo multisampleState =
			vkx::initializers::pipelineMultisampleStateCreateInfo(
		...
		vk::PipelineDynamicStateCreateInfo dynamicState =
			vkx::initializers::pipelineDynamicStateCreateInfo(
		...
		vk::GraphicsPipelineCreateInfo pipelineCreateInfo =
			vkx::initializers::pipelineCreateInfo(
		...
		vk::PipelineVertexInputStateCreateInfo emptyInputState = vkx::initializers::pipelineVertexInputStateCreateInfo();
		...
		std::array<vk::PipelineColorBlendAttachmentState, 3> blendAttachmentStates = {
			vkx::initializers::pipelineColorBlendAttachmentState(0xf, VK_FALSE),
			vkx::initializers::pipelineColorBlendAttachmentState(0xf, VK_FALSE),
			vkx::initializers::pipelineColorBlendAttachmentState(0xf, VK_FALSE)
		};
