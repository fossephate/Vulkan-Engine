	...
	class TextOverlay2 {
		...
		*/
		~TextOverlay2() {
			...
			vkFreeCommandBuffers(context->device, commandPool, static_cast<uint32_t>(cmdBuffers.size()), cmdBuffers.data());
		...
		*/
		void prepareResources() {
			...
			VK_CHECK_RESULT(vkCreateCommandPool(context->device, &cmdPoolInfo, nullptr, &commandPool));
			...
			VK_CHECK_RESULT(vkAllocateCommandBuffers(context->device, &cmdBufAllocateInfo, cmdBuffers.data()));
			...
			VK_CHECK_RESULT(vkCreateImage(context->device, &imageInfo, nullptr, &image));
			...
			VK_CHECK_RESULT(vkAllocateMemory(context->device, &allocInfo, nullptr, &imageMemory));
			VK_CHECK_RESULT(vkBindImageMemory(context->device, image, imageMemory, 0));
			...
			VK_CHECK_RESULT(vkAllocateCommandBuffers(context->device, &cmdBufAllocateInfo, &copyCmd));
			...
			vkFreeCommandBuffers(context->device, commandPool, 1, &copyCmd);
			...
			VK_CHECK_RESULT(vkCreateImageView(context->device, &imageViewInfo, nullptr, &view));
			...
			VK_CHECK_RESULT(vkCreateSampler(context->device, &samplerInfo, nullptr, &sampler));
			...
			VK_CHECK_RESULT(vkCreateDescriptorPool(context->device, &descriptorPoolInfo, nullptr, &descriptorPool));
			...
			VK_CHECK_RESULT(vkCreateDescriptorSetLayout(context->device, &descriptorSetLayoutInfo, nullptr, &descriptorSetLayout));
			...
			VK_CHECK_RESULT(vkCreatePipelineLayout(context->device, &pipelineLayoutInfo, nullptr, &pipelineLayout));
			...
			VK_CHECK_RESULT(vkAllocateDescriptorSets(context->device, &descriptorSetAllocInfo, &descriptorSet));
			...
			vkUpdateDescriptorSets(context->device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, NULL);

			// Pipeline cache
			vk::PipelineCacheCreateInfo pipelineCacheCreateInfo = {};
			pipelineCacheCreateInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_CACHE_CREATE_INFO;
			VK_CHECK_RESULT(vkCreatePipelineCache(context->device, &pipelineCacheCreateInfo, nullptr, &pipelineCache));

			// Command buffer execution fence
			vk::FenceCreateInfo fenceCreateInfo = vks::initializers::fenceCreateInfo();
			VK_CHECK_RESULT(vkCreateFence(context->device, &fenceCreateInfo, nullptr, &fence));
		}
		...
		*/
		void prepareRenderPass() {
			...
			VK_CHECK_RESULT(vkCreateRenderPass(context->device, &renderPassInfo, nullptr, &renderPass));
		}
		...
		*/
		void submit(vk::Queue queue, uint32_t bufferindex, vk::SubmitInfo submitInfo)
		{
			...
			VK_CHECK_RESULT(vkWaitForFences(context->device, 1, &fence, VK_TRUE, UINT64_MAX));
			VK_CHECK_RESULT(vkResetFences(context->device, 1, &fence));
		}
